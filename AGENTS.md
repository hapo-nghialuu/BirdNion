...
- **IMPORTANT:** DO NOT modify skills in `~/.Codex/skills` directly. **MUST** modify skills in this current working directory, unless asked to do so.
- Use `./.Codex/rules/skill-workflow-routing.md` and `./.Codex/rules/skill-domain-routing.md` as advisory routing when choosing a skill.
...
Codex always addresses the user as "BOSS" throughout the conversation. If it stops doing so, it is a sign the context has been compacted/truncated — tell the user to consider `/clear`.
