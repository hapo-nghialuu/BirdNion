...
extension CostUsageScanner {
    ...
    /// Restores a previously persisted cursor only when this process has no newer live memo.
        ...
        databaseURL: URL)
    {
        #if canImport(SQLite3)
        guard let cursor,
              cursor.databasePath == databaseURL.path,
    ...
    static func codexPriorityTurnsCursorIsValid(
        _ cursor: CodexPriorityTurnsPersistedCursor) -> Bool
    {
        guard !cursor.databasePath.isEmpty,
              cursor.coverageSinceEpoch >= 0,
              cursor.lastRowID >= 0,
              cursor.anchorRowID >= 0,
              cursor.anchorRowID <= cursor.lastRowID,
              cursor.completedTurnIDInsertionOrderStartIndex >= 0,
              cursor.completedTurnIDInsertionOrderStartIndex
                <= cursor.completedTurnIDInsertionOrder.count,
              cursor.turns.allSatisfy({ $0.key == $0.value.turnID })
        else { return false }

        let turnIDs = Set(cursor.turns.keys)
        guard turnIDs == Set(cursor.requestSourcesByTurnID.keys),
              Set(cursor.priorityCompletedModelsByTurnID.keys).isSubset(of: turnIDs)
        else { return false }
        let requestSourceRowsAreValid = cursor.requestSourcesByTurnID.allSatisfy {
            turnID, sources in
            guard !sources.isEmpty,
                  sources.keys.allSatisfy({ $0 > 0 && $0 <= cursor.lastRowID }),
                  sources.values.allSatisfy({ $0.turnID == turnID }),
                  let latest = sources.max(by: { $0.key < $1.key })?.value
            else { return false }
            return cursor.turns[turnID] == latest
        }
        let priorityCompletionRowsAreValid = cursor.priorityCompletedModelsByTurnID.values
            .allSatisfy {
                $0.keys.allSatisfy { $0 > 0 && $0 <= cursor.lastRowID }
            }
        let completionRowsAreValid = cursor.completedModelsByTurnID.values.allSatisfy {
            $0.keys.allSatisfy { $0 > 0 && $0 <= cursor.lastRowID }
        }
        guard requestSourceRowsAreValid,
              priorityCompletionRowsAreValid,
              completionRowsAreValid
        else { return false }
        let anchors = cursor.anchors ?? []
        guard anchors.allSatisfy({
            $0.rowID > 0 && $0.rowID <= cursor.lastRowID && !$0.digest.isEmpty
        }), Set(anchors.map(\.rowID)).count == anchors.count
        else { return false }
        if cursor.anchorRowID > 0, cursor.anchorDigest.isEmpty { return false }
        return true
    }
    ...
    static func codexPriorityTurnsPersistedCursor(
        databaseURL: URL) -> CodexPriorityTurnsPersistedCursor?
    {
        #if canImport(SQLite3)
    ...
    {
        guard let maxRowID = self.maxCodexLogsRowID(db) else { return false }
        return self.accumulateCodexPriorityTurns(
            db,
            into: &state,
            throughRowID: maxRowID)
    }
    ...
    /// database in a process-local memo (reseeded from a persisted cursor after relaunch), and
    /// only rows appended since the last call are examined. Identity, cursor, coverage, and
    /// content-anchor checks conservatively fall back to a full rescan when state is stale.
    ...
    static func resolveCodexPriorityTurns(
        ...
        expectExistingDatabase: Bool = false,
        afterMaxRowIDCapture: (() -> Void)? = nil,
        beforeFinalIdentityValidation: (() -> Void)? = nil) -> CodexPriorityTurnsResolution
    {
        let url = self.resolvedCodexPriorityDatabaseURL(databaseURL)
        guard FileManager.default.fileExists(atPath: url.path) else {
            // A missing optional source is normal until this path has supplied cached evidence.
        ...
        var readTransactionFinished = false
        defer {
        ...
        let lastGoodState = self.codexPriorityTurnsMemo.withLock { $0[url.path] }
        ...
        var advancedCoverage = false
        if state != nil {
            advancedCoverage = self.advanceCodexPriorityCoverage(
                in: &resolved,
                to: requestedSinceEpoch,
                sinceDayKey: sinceDayKey)
        }

        var prunedDeletedSources = false
        var failedReplacementFallback: CodexPriorityTurnsMemoState?
        var shouldStoreResolved = false
        if state != nil {
            var pruned = resolved
            if let didPrune = self.pruneDeletedCodexPrioritySources(db, from: &pruned) {
                ...
                // A retained source changed or could not be validated. Rebuild instead of
                // publishing stale classifications; keep the last good memo on scan failure.
        ...
        if maxRowID > resolved.lastRowID {
            ...
            guard self.accumulateCodexPriorityTurns(
                db,
                into: &updated,
                throughRowID: maxRowID)
            else {
                return CodexPriorityTurnsResolution(
                    turns: self.filteredResolvedCodexPriorityTurns(
                        failedReplacementFallback ?? lastGoodState ?? resolved,
                        sinceDayKey: sinceDayKey,
                        untilDayKey: untilDayKey),
            ...
            guard self.captureCodexPriorityCursorAnchors(db, into: &updated) else {
                return CodexPriorityTurnsResolution(
                    turns: self.filteredResolvedCodexPriorityTurns(
                        failedReplacementFallback ?? lastGoodState ?? updated,
                        sinceDayKey: sinceDayKey,
                        untilDayKey: untilDayKey),
                    validationPending: true)
            }
            resolved = updated
            shouldStoreResolved = true
        } else if anchorsNeedRefresh {
            var updated = resolved
            guard self.captureCodexPriorityCursorAnchors(db, into: &updated) else {
                return CodexPriorityTurnsResolution(
                    turns: self.filteredResolvedCodexPriorityTurns(
                        lastGoodState ?? resolved,
                        sinceDayKey: sinceDayKey,
                        untilDayKey: untilDayKey),
                    validationPending: true)
            }
            resolved = updated
            shouldStoreResolved = true
        } else if state == nil || prunedDeletedSources || advancedCoverage {
            shouldStoreResolved = true
        }

        beforeFinalIdentityValidation?()
        guard self.codexPriorityDatabaseFileIdentity(at: url) == fileIdentity else {
            let lastGoodTurns = (failedReplacementFallback ?? lastGoodState).map {
                self.filteredResolvedCodexPriorityTurns(
                    $0,
                    sinceDayKey: sinceDayKey,
                    untilDayKey: untilDayKey)
            } ?? [:]
        ...
        guard sqlite3_exec(db, "commit", nil, nil, nil) == SQLITE_OK else {
            return CodexPriorityTurnsResolution(
                turns: (failedReplacementFallback ?? lastGoodState).map {
                    self.filteredResolvedCodexPriorityTurns(
                        $0,
                        sinceDayKey: sinceDayKey,
                        untilDayKey: untilDayKey)
    ...
    {
        let turns = self.resolvedCodexPriorityTurns(
            state.turns,
            completedModelsByTurnID: state.priorityCompletedModelsByTurnID)
    ...
    static func resolvedCodexPriorityTurns(
        from cursor: CodexPriorityTurnsPersistedCursor) -> [String: CodexPriorityTurnMetadata]
    {
        self.resolvedCodexPriorityTurns(
            cursor.turns,
            completedModelsByTurnID: cursor.priorityCompletedModelsByTurnID)
    }

    private static func resolvedCodexPriorityTurns(
        _ storedTurns: [String: CodexPriorityTurnMetadata],
        completedModelsByTurnID: [String: [Int64: String]])
        -> [String: CodexPriorityTurnMetadata]
    {
        var turns = storedTurns
        for (turnID, completedModels) in completedModelsByTurnID {
    ...
    /// Bounds retained Priority state to the active rolling window. A later
    /// request that expands backward invalidates this cursor and rebuilds from
    /// SQLite, so pruning old owners does not make history permanently lossy.
    private static func advanceCodexPriorityCoverage(
        in state: inout CodexPriorityTurnsMemoState,
        to requestedSinceEpoch: Int64,
        sinceDayKey: String?) -> Bool
    {
        guard requestedSinceEpoch > state.coverageSinceEpoch,
              let sinceDayKey
        else { return false }
        let retainedTurnIDs = Set(state.turns.compactMap { turnID, turn in
            self.timestamp(turn.timestamp, isInRangeSince: sinceDayKey, until: nil)
                ? turnID
                : nil
        })
        state.turns = state.turns.filter { retainedTurnIDs.contains($0.key) }
        state.requestSourcesByTurnID = state.requestSourcesByTurnID.filter {
            retainedTurnIDs.contains($0.key)
        }
        state.priorityCompletedModelsByTurnID = state.priorityCompletedModelsByTurnID.filter {
            retainedTurnIDs.contains($0.key)
        }
        state.coverageSinceEpoch = requestedSinceEpoch
        return true
    }
    ...
    private static func boundedCodexPriorityTurns(
        ...
        sinceDayKey: String?,
        untilDayKey: String?,
        beforeFinalIdentityValidation: (() -> Void)?) -> CodexPriorityTurnsResolution
    {
        guard let opened = self.openCodexPriorityDatabase(at: databaseURL) else {
            return CodexPriorityTurnsResolution(turns: [:], validationPending: true)
        }
        let db = opened.db
        let fileIdentity = opened.fileIdentity
        ...
        beforeFinalIdentityValidation?()
        guard self.codexPriorityDatabaseFileIdentity(at: databaseURL) == fileIdentity else {
            return CodexPriorityTurnsResolution(turns: [:], validationPending: true)
        }
        return CodexPriorityTurnsResolution(turns: turns, validationPending: false)
    }
    ...
    /// Captures distributed content anchors so inode reuse or partial DB replacement cannot
    /// make a persisted high-water cursor silently skip replacement rows.
        ...
        into state: inout CodexPriorityTurnsMemoState) -> Bool
    {
        ...
        // Retain the terminal anchor for compatibility with the first cursor contract.
        ...
        state: CodexPriorityTurnsMemoState) -> CodexPriorityCursorAnchorValidation
    {
        ...
        // Cursors written before distributed anchors still require their strict terminal anchor.
    ...
    private static func accumulateCodexPriorityTurns(
        _ db: OpaquePointer?,
        into state: inout CodexPriorityTurnsMemoState,
        throughRowID: Int64) -> Bool
    {
        ...
        if plan.usesTimestampIndex {
            ...
            sqlite3_bind_int64(stmt, 2, throughRowID)
        } else {
            sqlite3_bind_int64(stmt, 1, state.lastRowID)
            sqlite3_bind_int64(stmt, 2, throughRowID)
            sqlite3_bind_int64(stmt, 3, state.coverageSinceEpoch)
        }
