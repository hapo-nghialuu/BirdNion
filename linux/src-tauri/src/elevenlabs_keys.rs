//! ElevenLabs multi-key store — same shape as the FreeModel account store:
//! managed API keys live in their own file
//! (`~/.config/birdnion/elevenlabs-keys.json`, chmod 0600); only the active
//! key id rides in settings.json (`active_elevenlabs_key`).
//!
//! Legacy single `providers.elevenlabs.apiKey` is imported once when the
...
fn metadata_path() -> Option<PathBuf> {
    config::support_dir().map(|path| path.join("elevenlabs-keys.json"))
}
...
fn to_key(e: &Entry) -> ElevenLabsKey {
    ElevenLabsKey {
        id: e.id.clone(),
        label: e.label.clone(),
...
/// One-time import of legacy `providers.elevenlabs.apiKey` when the multi-key
...
pub fn ensure_legacy_import() {
    ...
    let Some(provider) = settings.providers.iter().find(|p| p.id == "elevenlabs") else {
        return;
    };
...
pub fn active_id() -> Option<String> {
    ...
    let settings = config::load();
    if let Some(id) = settings.active_elevenlabs_key {
...
pub fn set_active(id: &str) -> Result<(), String> {
    config::update(|settings| {
        settings.active_elevenlabs_key = Some(id.to_string());
        Ok(())
    })
}
...
pub fn all_keys() -> Vec<ElevenLabsKey> {
    ensure_legacy_import();
...
pub fn add(api_key: &str, label: Option<&str>) -> Result<ElevenLabsKey, String> {
    ...
    entries.push(entry);
    persist(&entries)?;
    if is_first || config::load().active_elevenlabs_key.is_none() {
        set_active(&key.id)?;
    }
    Ok(key)
}
...
pub fn remove(id: &str) -> Result<(), String> {
    ...
    if previous.as_deref() == Some(id) {
        if let Some(first) = remaining.first() {
            ...
            config::update(|settings| {
                settings.active_elevenlabs_key = None;
                Ok(())
            })?;
        }
    }
