...
export function elevenlabsKeysSection(): HTMLElement {
  ...
  const notifyChanged = async (state: ElevenLabsKeysState) => {
    ...
    for (const key of state.keys) {
      ...
      if (key.id === state.activeId) {
        ...
        useBtn.addEventListener("click", async () => {
          try {
            const next = await invoke<ElevenLabsKeysState>("elevenlabs_key_switch", { id: key.id });
      ...
      removeBtn.addEventListener("click", async () => {
        try {
          const next = await invoke<ElevenLabsKeysState>("elevenlabs_key_remove", { id: key.id });
          await notifyChanged(next);
          await render();
        } catch (err) {
          status.textContent = `${t("loadError")}: ${err}`;
  ...
  addBtn.addEventListener("click", async () => {
    ...
    addBtn.textContent = "…";
    try {
      const next = await invoke<ElevenLabsKeysState>("elevenlabs_key_add", {
        apiKey,
