...
| Quota Agenda MVP | All tab tối đa 3 dòng, chọn reset tương lai gần nhất, state `scheduled` / `awaitingRefresh` / `unknown` / `staleLastKnown`, source/account/freshness privacy-safe, click đi provider tab | `QuotaAgendaProjection` + `AllAgentsQuotaSection`; `quota-agenda.ts` + `quota-agenda-section.ts`; `configured-agents-section.ts` giữ sibling slot đồng bộ; provider/privacy event refresh hai capability slot | **done** — percent vẫn là giá trị đã normalize từ provider; native-unit / overage contract chưa nằm trong MVP; Linux targeted DOM có automated receipt nhưng chưa có visual E2E receipt |
