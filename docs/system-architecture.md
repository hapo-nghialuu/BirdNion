...
- Linux Codex scanner cùng giữ last-good/seed-first: command trả ngay `cost-history.json`, một worker singleflight chạy nền theo episode tối đa 2 giây rồi resume từ journal v3. Frontend đăng ký completion listener trước request đầu, fence theo request epoch + generation + canonical source và bỏ event trùng/cũ.
- JSONL Linux đóng băng manifest/EOF, mỗi pass tối đa 500 ms, 16 MiB và 4 MiB/file. Record đã chuẩn hoá được checkpoint vào `codex-scan-spool.sqlite` private `0600`, `O_NOFOLLOW`, WAL + synchronous FULL; key chỉ gồm generation, locator hash và sequence, không persist raw path/session/turn. Aggregation stream từ SQLite, giữ đúng fork baseline, duplicate winner, project retraction và Priority pricing mà không materialize toàn bộ event timeline trong RAM.
- `logs_2.sqlite` Priority được đọc read-only theo row/byte/time budget, đóng băng high-water và xác minh digest mọi source row trước lần publish cold. Row hiện tại vượt 1 MiB, DB/path bị thay, malformed state hoặc scan incomplete đều fail-closed và giữ last-good; source đã hết hạn được bỏ có kiểm soát.
