...
- [x] [both] Quota Agenda MVP — All tab tối đa 3 dòng, ưu tiên reset tương lai gần nhất, state `scheduled` / `awaitingRefresh` / `unknown` / `staleLastKnown`, metadata source/account/freshness privacy-safe, click đi provider tab; Linux refresh đồng thời slot Agenda/Đã cấu hình khi quota hoặc privacy đổi, không rerender chart — 2026-08-30
...
| Date | Milestone |
|---|---|
| 2026-08-30 | Quota Agenda MVP implemented on macOS/Linux All tab: max 3 rows, nearest explicit future reset, privacy-safe source/account/freshness, provider-tab drilldown; Linux uses targeted Agenda/configured slot repaint on provider updates and `birdnion-hide-personal-info-changed`. |
