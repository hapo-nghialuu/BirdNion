...
| Date | Milestone |
|---|---|
| 2026-09-02 | Linux Codex local-usage parity: seed-first last-good, singleflight background completion, bounded JSONL passes + durable journal v3/SQLite spool, incremental warm append, exact streaming fork/duplicate/project retraction, Priority SQLite frozen high-water + full source validation và frontend stale-generation fence. Gate trên macOS host: Codex `70/70`, Rust `675/675`, Node `93/93`, production build `65` modules; Linux GUI/runtime E2E vẫn cần receipt trên Linux thật. |
| 2026-08-31 | Hotfix local-usage Codex: frozen finite generation (immutable range/manifest/record EOF), committed và pending maps tách biệt, durable completion receipt + catch-up, main/parent resume theo `parsedBytes`, oversized-line discard journal, bounded metadata prepass, no-follow regular-file `openat`, canonical path identity/alias rollback, overflow-safe progress, progress-driven singleflight và same-day last-good/high-water. Frozen target unavailable rollback toàn working generation và giữ nguyên trust metadata; reopen/day change chỉ retrigger khi dropdown visible và tab All/Codex. Gate: Codex `68/68`; full macOS `813` executed (`812` pass, `1` live skip, `0` fail); review độc lập `PASS 9.5/10`; visual runtime được kiểm riêng sau khi mở Debug app. |
