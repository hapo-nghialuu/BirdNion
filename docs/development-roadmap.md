...
| Date | Milestone |
|---|---|
| 2026-08-30 | Quota Agenda MVP implemented as a macOS/Linux companion panel opened by the popover footer calendar icon; existing All-tab Quota/Configured blocks remain unchanged. Max 3 rows, nearest explicit future reset, privacy-safe source/account/freshness, close-then-validate provider drilldown, and updates ride existing status/tick/privacy/catalog events without a new poller. |
